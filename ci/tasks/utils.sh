...
function read_infrastructure {
  ...
  export google_jumpbox_ip=$(cat ${infrastructure_metadata} | jq -r .jumpbox_ip)
  export google_internal_cidr=$(cat ${infrastructure_metadata} | jq -r .internal_cidr)
  export google_internal_gw=$(cat ${infrastructure_metadata} | jq -r .internal_gw)
  export google_internal_jumpbox_ip=$(cat ${infrastructure_metadata} | jq -r .internal_jumpbox_ip)
}
