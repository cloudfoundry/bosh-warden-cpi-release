...
func (m *Metadata) IsLightStemcell() bool {
	// Check if stemcell_formats contains "docker-light" or "warden-light"
	for _, format := range m.StemcellFormats {
		if format == "docker-light" || format == "warden-light" {
			return true
		}
	}
	return false
...
// GetImageReference returns the OCI image reference from metadata
