// This file was generated by counterfeiter
package fakes

import (
	"sync"

	"github.com/cppforlife/bosh-cpi-go/apiv1"
	"github.com/cppforlife/bosh-warden-cpi/stemcell"
)

type FakeFinder struct {
	FindStub        func(apiv1.StemcellCID) (stemcell.Stemcell, bool, error)
	findMutex       sync.RWMutex
	findArgsForCall []struct {
		arg1 apiv1.StemcellCID
	}
	findReturns struct {
		result1 stemcell.Stemcell
		result2 bool
		result3 error
	}
	invocations      map[string][][]interface{}
	invocationsMutex sync.RWMutex
}

func (fake *FakeFinder) Find(arg1 apiv1.StemcellCID) (stemcell.Stemcell, bool, error) {
	fake.findMutex.Lock()
	fake.findArgsForCall = append(fake.findArgsForCall, struct {
		arg1 apiv1.StemcellCID
	}{arg1})
	fake.recordInvocation("Find", []interface{}{arg1})
	fake.findMutex.Unlock()
	if fake.FindStub != nil {
		return fake.FindStub(arg1)
	} else {
		return fake.findReturns.result1, fake.findReturns.result2, fake.findReturns.result3
	}
}

func (fake *FakeFinder) FindCallCount() int {
	fake.findMutex.RLock()
	defer fake.findMutex.RUnlock()
	return len(fake.findArgsForCall)
}

func (fake *FakeFinder) FindArgsForCall(i int) apiv1.StemcellCID {
	fake.findMutex.RLock()
	defer fake.findMutex.RUnlock()
	return fake.findArgsForCall[i].arg1
}

func (fake *FakeFinder) FindReturns(result1 stemcell.Stemcell, result2 bool, result3 error) {
	fake.FindStub = nil
	fake.findReturns = struct {
		result1 stemcell.Stemcell
		result2 bool
		result3 error
	}{result1, result2, result3}
}

func (fake *FakeFinder) Invocations() map[string][][]interface{} {
	fake.invocationsMutex.RLock()
	defer fake.invocationsMutex.RUnlock()
	fake.findMutex.RLock()
	defer fake.findMutex.RUnlock()
	return fake.invocations
}

func (fake *FakeFinder) recordInvocation(key string, args []interface{}) {
	fake.invocationsMutex.Lock()
	defer fake.invocationsMutex.Unlock()
	if fake.invocations == nil {
		fake.invocations = map[string][][]interface{}{}
	}
	if fake.invocations[key] == nil {
		fake.invocations[key] = [][]interface{}{}
	}
	fake.invocations[key] = append(fake.invocations[key], args)
}

var _ stemcell.Finder = new(FakeFinder)
