...
var _ = Describe("CreateVM", func() {
	...
	Describe("Run", func() {
		...
		It("tries to find stemcell with given stemcell cid", func() {
			...
			_, err := action.Run("fake-agent-id", stemcellCID, cloudProperties, networks, diskLocality, env)
		...
		Context("when stemcell is found with given stemcell cid", func() {
			...
			It("returns id for created VM", func() {
				...
				id, err := action.Run("fake-agent-id", stemcellCID, cloudProperties, networks, diskLocality, env)
				Expect(err).ToNot(HaveOccurred())
			...
			It("creates VM with requested agent ID, stemcell, and networks", func() {
				...
				Expect(vmCreator.CreateCloudProperties).To(Equal(cloudProperties.AsVMCloudProperties()))
